...
function uploadFile(targets, id, vField, isRequired, maxSize, minSize) {
  ...
  try {
    ...
    var action = '/documents/upload-document';
...
function removeTableRow(tableID, removeNum) {
  ...
  if (current_total_row <= 3) {
    var tableFooter = document.getElementById('autoFooter');
    if (tableFooter) {
      tableFooter.remove();
    }
  }
...
function bd_currency_format(data, decimal_part) {
  var decimal = parseInt(decimal_part);
  var currency_amount = parseFloat(data).toFixed(decimal);
  var split_data = (currency_amount + "").split(".");
  var left_part = split_data[0];
  var fraction_part = split_data[1];
  var left_part_length = left_part.length;
  if (left_part_length) {
    var str = '';
    for (var i = 0, pointer = 1; i < left_part_length; i++, pointer++) {
